...
WITH LatestSalePriceCTE AS (
    ...
    WHERE it.SalePrice IS NOT NULL AND it.TransactionTypeId = 1 -- 1 = Sales
),
StockCTE AS (
    SELECT
        it.ProductVariantId,
        SUM(CASE WHEN it.TransactionTypeId = 0 THEN it.Quantity ELSE -it.Quantity END) as CurrentStock -- 0 = Purchase
    FROM InventoryTransactions it
...
SELECT
    ...
    pv.Color,
    pv.Size,
    COALESCE(s.CurrentStock, 0) AS Stock,
    lsp.SalePrice AS LastSalePrice
FROM Products p
...
ORDER BY p.Name, pv.Color, pv.Size;";
        ...
        return await _dapper.QueryAsync<ProductSearchResultDto>(
            sql,
            parameters,
